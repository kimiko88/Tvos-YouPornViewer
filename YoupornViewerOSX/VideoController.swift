...
class VideoController: NSViewController {
    ...
    var bounds: NSRect! = NSRect(x: 0, y: 0, width: 700, height: 800)
    var video: Video!
...
//        self.view.backgroundColor = UIColor.blackColor()
    ...
    override func prepareForSegue(segue: NSStoryboardSegue, sender: AnyObject!) {
        if segue.identifier == "BackHome"{
            _ = segue.destinationController as! ViewController
        }else{
            let video = sender as! DownloadLink
            if segue.identifier == "SeeVideo"{
                let vc = segue.destinationController as! AVVideoPlayerController
